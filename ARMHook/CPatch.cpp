...
namespace ARMHook
{
	...
	void CPatch::SetInt(uintptr_t addr, int value)
	{
		CHook::WriteMemory((void*)addr, &value, sizeof(value));
	}
	
	void CPatch::SetUintptr(uintptr_t addr, uintptr_t value)
	{
		CHook::WriteMemory((void*)addr, &value, sizeof(value));
	}
	...
	int CPatch::GetInt(uintptr_t addr)
	{
		int value;
		CHook::ReadMemory((void*)addr, &value, sizeof(int));
		return value;
	}
	
	uintptr_t CPatch::GetUintptr(uintptr_t addr)
	{
		uintptr_t value;
		CHook::ReadMemory((void*)addr, &value, sizeof(uintptr_t));
		return value;
